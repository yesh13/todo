...
var actList=app.controller("actListCtrl",['dragService','$scope','$http','$stateParams',function(dragService,$scope,$http,$stateParams){
	var actListCtrl=this;
	$scope.activity={};
	$scope.actNum=0;
	$scope.initList=function(){
		console.log("init list");
		$http.get("/api/activity/list/"+$stateParams.aid)
	    .success(function(response) {
	    	$scope.actList = response;
	    	dragService.list=$scope.actList;
	    	if($scope.actList.length>0){
	    		$scope.setActivity($scope.actNum);
	    	}
	    	$http.get("/api/activity/path/"+$stateParams.aid)
		    .success(function(response) {
	...
	$scope.setActivity=function(n){
		if(n<0){
			...
			$http.get("/api/activity/detail/"+aid)
		    .success(function(response) {
	...
	$scope.sumbit=function(){
		if($scope.activity.aid==null){
			...
			console.log("new: "+JSON.stringify($scope.activity));
			$http.post("/api/activity",$scope.activity).success(function(){
				$scope.initList();
			});
		}else{
			console.log("edit: "+JSON.stringify($scope.activity));
			$http.post("/api/activity/"+$scope.activity.aid,$scope.activity).success(function(){
				$scope.initList();
			});
		}
	};
...
app.directive('droppableActivity', ['$http',function($http) {
	return function(scope, element, attr) {
		element.on('drop', function(event) {
			// Prevent default dragging of selected content
			var droped=scope.$eval(attr.droppableActivity);
			console.log(scope.beingDragged+"->"+droped);
			var move={parent:droped};
			$http.post("/api/activity/"+scope.beingDragged,move).success(function(){
				scope.initList();
			});
		});
